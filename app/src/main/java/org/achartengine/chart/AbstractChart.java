    ...
    protected void drawLegend(Canvas canvas, DefaultRenderer defaultRenderer, String[] strArr, int i, int i2, int i3, int i4, int i5, int i6, Paint paint) {
        if (defaultRenderer.isShowLegend()) {
            ...
            for (int i7 = 0; i7 < min; i7++) {
                ...
                f3 = f + f42;
                if (i7 > 0 && getExceed(f3, defaultRenderer, i2, i4)) {
                    f = (float) i;
                    f2 += 15.0f;
                    f3 = f + f42;
                }
                if (getExceed(f3, defaultRenderer, i2, i4)) {
                    f3 = ((((float) i2) - f) - legendShapeWidth) - 10.0f;
                    if (isVertical(defaultRenderer)) {
                        f3 = ((((float) i4) - f) - legendShapeWidth) - 10.0f;
                    }
                    str = str2.substring(0, paint.breakText(str2, true, f3, fArr)) + "...";
                } else {
                    str = str2;
                }
