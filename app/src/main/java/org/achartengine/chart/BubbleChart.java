...
public class BubbleChart extends XYChart {
    ...
    public void drawSeries(Canvas canvas, Paint paint, float[] fArr, SimpleSeriesRenderer simpleSeriesRenderer, float f, int i) {
        ...
        for (int i2 = 0; i2 < length; i2 += MIN_BUBBLE_SIZE) {
            double value = xYValueSeries.getValue(i2 / MIN_BUBBLE_SIZE);
            drawCircle(canvas, paint, fArr[i2], fArr[i2 + 1], (float) ((value * maxValue) + 2.0d));
        }
    }
